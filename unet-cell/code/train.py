...
def train(args):
    ...
    for epoch in range(args.n_epochs):
        for i, (img, label, img_names) in enumerate(train_loader):
            ...
            loss = criterion(pred, label) #weighted_binary_cross_entropy(pred, label, pos_weight)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if batch % args.snap_interval == 0 and batch > 0:
                ...
                snap += 1
            batch += 1
        np.savetxt('../results/train_log.csv', train_log, delimiter=",")
        np.savetxt('../results/snap_log.csv', snap_log, delimiter=",")
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'loss': loss,
            }, '../model/model'+str(epoch+1))
...
def validation(args):
    ...
    if cuda:
        model = UNet(3, 1).cuda(args.gpu)
        checkpoint = torch.load('../model/model')
